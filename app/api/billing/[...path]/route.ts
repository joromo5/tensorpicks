...
/**
 * Catch-all proxy for /api/billing/* → engine /billing/*
 *
 * Forwards the Clerk JWT and request body to the Python engine,
 * supporting all HTTP methods used by the billing router.
 */

function extractPath(req: NextRequest): string {
  const url = new URL(req.url);
  // Strip /api prefix to get the engine path: /api/billing/... → /billing/...
  const match = url.pathname.match(/^\/api(\/billing\/.*)$/);
  return match?.[1] ?? "/billing";
}
